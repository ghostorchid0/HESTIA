...
async function sendTrialReminders() {
  ...
  for (const hotel of hotels) {
    await notifySubscriptionEvent(
      ...
      `Bonjour ${hotel.name}, votre essai Hestia expire dans ${REMINDER_DAYS} jours. Achetez une licence sur Chariow pour ${config.billing.customerPrice} ${config.billing.currency} et continuez à recevoir des commandes.`
    );
  }
}
...
async function sendRenewalReminders() {
  ...
  for (const hotel of hotels) {
    await notifySubscriptionEvent(
      ...
      `Bonjour ${hotel.name}, votre abonnement Hestia expire dans ${REMINDER_DAYS} jours. Achetez une nouvelle licence sur Chariow pour éviter une interruption.`
    );
  }
}
...
module.exports = { startBillingJobs, runDailyBillingJobs };
